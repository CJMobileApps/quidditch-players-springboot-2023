...
import com.cjmobileapps.quidditchplayers.data.model.*
...
class QuidditchplayersApplicationTest : BaseIntegrationTest() {
    ...
    @Test
    fun `get houses, players the player status happy flow`(): Unit = runBlocking {
        ...
        val houseApiResponse = withContext(Dispatchers.IO) {
        ...
        val playerUri = UriComponentsBuilder.fromHttpUrl("${localHostUri}/api/v1/quidditchplayers/player")
            .queryParam("houseName", ravenClawHouse?.name?.name)
            .toUriString()

        // when playersApiResponse
        val playersApiResponse = withContext(Dispatchers.IO) {
            restTemplate.exchange(
                playerUri,
                HttpMethod.GET,
                httpEntity,
        ...
        val positionsApiResponse = withContext(Dispatchers.IO) {
            restTemplate.exchange(
        ...
        val statusUri = UriComponentsBuilder.fromHttpUrl("${localHostUri}/api/v1/quidditchplayers/player/status/${player?.id}")
            .toUriString()

        // when playersApiResponse
        val statusApiResponse = withContext(Dispatchers.IO) {
            restTemplate.exchange(
                statusUri,
                HttpMethod.GET,
                httpEntity,
